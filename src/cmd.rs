    ...
    if args.is_empty() {
        ...
        match args[0].to_lowercase().as_str() {
            ...
            create -netcat <name> <port>";
                ...
                if used_port.contains(&port) { println!("please use a different port, port is being utilized") } else if used_name.contains(&name) { println!("please use a different name, name is being utilized") } else {
...
pub(crate) fn cmd(session_handler: &mut SessionHandler, args: Vec<String>) {
    ...
    if args.is_empty() {
        ...
        println!("you are sending \"{}\" to the active session", command_string);
        session_handler.send_command(command_string);
    }
}
